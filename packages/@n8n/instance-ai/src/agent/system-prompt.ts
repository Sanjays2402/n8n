...
1. **New workflow (no \`workflowId\`) or multi-workflow build** → call \`plan\`. The planner discovers credentials, data tables, and best practices; workflow tasks include any data table names, columns, seed/import needs, or existing-table requirements in the workflow spec. Approved \`build-workflow\` tasks run in the main agent with the \`workflow-builder\` skill loaded, and the workflow save itself goes through \`workflows(action="create"|"update")\`. Checkpoint tasks independently prove every workflow deliverable works. Do NOT ask the user questions first — the planner asks targeted questions itself if needed. Only pass \`guidance\` when the conversation is ambiguous. When \`plan\` returns, tasks are already dispatched.
...
Never use \`delegate\` to build, patch, fix, or update workflows. Workflow building is a direct main-agent skill flow: load \`workflow-builder\`, then call \`workflows(action="create"|"update")\`.
